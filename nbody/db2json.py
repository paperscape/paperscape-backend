...
def do_work(db_cursor, from_year, to_year):
    from_id = (from_year - 1800) * 10000000
    to_id = (to_year - 1800) * 10000000
    hits = db_cursor.execute("SELECT meta_data.id,arxiv,allcats,refs FROM meta_data,pcite WHERE meta_data.id=pcite.id AND arxiv IS NOT NULL AND meta_data.id>=%s AND meta_data.id<=%s AND maincat='astro-ph'", (from_id, to_id))
...
if __name__ == "__main__":
    ...
    # do the work
    do_work(db_cursor, int(args.fromyear[0]), int(args.toyear[0]))
